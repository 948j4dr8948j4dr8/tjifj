...
<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0f2027,50:203a43,100:2c5364&height=180&section=header&text=X-UI%20Multi-Country%20Gateway&fontSize=38&fontColor=ffffff&animation=fadeIn&fontAlignY=38&desc=Direct%20egress%20%2B%20per-country%20exits%20%E2%80%94%20one%20public%20port&descAlignY=58&descSize=16" width="100%"/>
...
[![YouTube](https://img.shields.io/badge/YouTube-X4G%20HUB-FF0000?style=for-the-badge&logo=youtube&logoColor=white)](https://www.youtube.com/@X4GHUB)

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=18&duration=2600&pause=900&color=39D353&center=true&vCenter=true&width=700&lines=Single+public+port+%E2%80%94+everything+behind+nginx+%3A3000;Fast%2C+multi-provider+country+discovery;Failed+countries+are+never+half-created;Automatic+per-country+IP+switching" alt="typing banner"/>

</div>

<br/>

> **This revision fixes the root cause of the `address already in use` crash loop**: the "direct" (non-Tor) inbound and nginx were both trying to bind **8080** while nginx *also* listened on 3000 — on single-external-port hosts (Railway and similar) only one of those was ever actually reachable. Now **nginx is the only thing bound to a public port (`3000`)**; the direct inbound is internal-only (`127.0.0.1:8080`) and nginx proxies to it. Nothing else changes about how you use the panel or the endpoints.
...
[![X4G HUB on YouTube](https://img.shields.io/badge/Subscribe-X4G%20HUB-FF0000?style=for-the-badge&logo=youtube&logoColor=white)](https://www.youtube.com/@X4GHUB)

**[youtube.com/@X4GHUB](https://www.youtube.com/@X4GHUB)** — deployment walkthroughs, config breakdowns, and updates for this project.
...
Before: `nginx` listened on `3000` **and** the xray direct inbound also tried to `listen 0.0.0.0:8080`. On a platform that only forwards **one** external port to your container, that second bind either collides with something else on the same port or is simply never reachable from outside — the logs showed exactly this:
...
After: `nginx` is the **only** process bound to `0.0.0.0`, on port `3000`. The direct inbound binds `127.0.0.1:8080` (loopback-only) and nginx's `/` and `/direct` locations reverse-proxy to it. Every Tor country inbound, the panel, and Tor's own SOCKS/Control ports all follow the same rule — 127.0.0.1 only, reached exclusively via an nginx `location` block.
...
| **Country discovery** | Sequential, 1 provider timeout path, up to 15 retries × (10s + 15s) sleeps ≈ minutes per country | **Parallel across all countries**, 4 geo-IP providers + 5 IP-echo services tried per attempt, short bounded timeouts, tunable via `config.json` |
| **Failed countries** | Mostly already excluded, but inconsistently | **Single source of truth** (`is_location_verified()`): a failed country gets **no** inbound, **no** client, **no** outbound, **no** routing rule, and **no** nginx location — enforced identically in `start.sh`, `panel-bootstrap.sh`, and the nginx template |
| **Naming in panel** | Inbound tags/remarks were `Tor-Germany`, outbound tag `tor-de` | Tags/remarks use **only the country** — `Germany` / `de`. The word "Tor" never appears in any inbound, client, outbound, or routing rule |
| **IP rotation** | Rotator existed but was a single hardcoded loop | Same rotator, now clearly documented as **automatic IP switching per verified country**, tunable interval, wrong-country self-healing on rotation |
| **nginx config** | 10 country blocks hardcoded, always present even for failed/removed countries | Country location blocks are **generated dynamically** from the verified-country list at container start |
...
4. If the first rotation lands in the wrong country, one more attempt is made immediately; if that also fails, the country is marked unreachable until the *next* scheduled rotation (it is **not** torn down from the panel for a single bad rotation — only initial-discovery failures result in removal).
...
- **Excluded regions** — `tor.exclude_countries` in `config.json` (oppressive-regime and high-risk jurisdictions) are excluded from every instance's possible exit set, not just the target country's own instance.
- **No "Tor" in the panel** — inbound tags, remarks, outbound tags, and routing rules use only the country code/label. Panel screenshots, exported client links, and the xray JSON config never contain the word "Tor".
...
[![X4G HUB](https://img.shields.io/badge/X4G%20HUB-Subscribe-FF0000?style=for-the-badge&logo=youtube&logoColor=white)](https://www.youtube.com/@X4GHUB)

New deployments, troubleshooting sessions, and config walkthroughs for this project are posted at **[youtube.com/@X4GHUB](https://www.youtube.com/@X4GHUB)**.
